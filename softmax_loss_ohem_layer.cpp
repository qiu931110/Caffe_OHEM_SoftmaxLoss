...
void SoftmaxWithLossOHEMLayer<Dtype>::Reshape(
  ...
  use_hard_mining_ = this->layer_param_.softmax_param().use_hard_mining();
  batch_size_ = this->layer_param_.softmax_param().batch_size();
  CHECK_GT(batch_size_, 0);
  int temp_size = inner_num_ * outer_num_;
  batch_size_ = std::min(batch_size_, temp_size);
  Dtype hard_ratio = this->layer_param_.softmax_param().hard_ratio();
